...
2. **Make the deck.** `node bin/deck.mjs new "<Title>" --in <folder> --description "…"` (ask which folder if unclear)
3. **Outline.** One idea per slide (more slides are fine): title → the problem → the idea → how it
   ...
   `decks/…/<deck>/slides/manifest.json`.
4. **Draw.** One file per slide in `decks/…/<deck>/slides/`, using the kit (`docs/drawing-slides.md`).
   Hand-made, minimal, ink on paper, one red accent, reveals on clicks (`beat` + `anim`), speaker notes
   in the checklist format, a `source` line on data slides. Learn from `decks/weekly-presentations/seeing-is-fixing/slides/`.
   For big decks, split the slide files across parallel helpers, each testing in its own deck copy.
5. **Build and look.** `node bin/deck.mjs build <deck>`, then screenshot every slide through the
   tldraw Desktop API and fix what you see (`--only` to rebuild one). Present through some clicks.
6. **Finish.** `node bin/deck.mjs cover <deck>` and `node bin/deck.mjs check <deck>`. Commit and push
   only when the user asks; the push publishes it to the website.

## Changing an existing deck

- Code slides: edit `decks/…/<deck>/slides/*.js` → `deck build <deck> --only <file>`.
- Its own code (scenes, actions, templates): `decks/…/<deck>/ext/` → `deck install <deck>`.
- Title, description, hidden: `deck.json` (keep it to those). Organising (folders, moving): `deck folder`, `deck move`, or the studio.
